...
const Managestock:React.FC = () => {
  ...
  const [productCount, setProductCount] = useState<ProductCountStatus[] | null>(null);
  ...
  useEffect(() => {
    ...
    axios({
      ...
      data: {
        ...
        product_name: filter.searchAction == 'productName' ? filter.searchField: null,
        product_sku: filter.searchAction == 'productSku' ? filter.searchField: null,
  ...
  .map(([key, headerData]: [string, HeaderType]): ColumnDef<Record<string, any>, any> => ({
    ...
    cell: ({ row }) => {
      ...
      let value = headerName === "Product" ? row.original.name : row.original[key];
      let type = headerData.type;

      const isBackOrdersOrStock = headerName === "Back orders" || headerName === "Stock";
      ...
      if (isBackOrdersOrStock) {
        if (!manageStock) {
          type = "";
          if (headerName === "Back orders" && headerData.options) {
            value = headerData.options[value as keyof typeof headerData.options];
          }
        }
