...
export type SettingContent = {
    ...
    if (Array.isArray(settings)) {
      settings.sort((firstSet, secondSet) => {
        ...
        if (firstSet.type === 'folder') {
          firstSet.content = getSettingsByPriority(firstSet.content as Setting[]);
          const firstChild = (firstSet.content as Setting[])[0];
          firstPriority = ((firstChild.content as SettingContent).priority || 0);
        } else {
          firstPriority = (firstSet.content as SettingContent).priority;
        }
  
        if (secondSet.type === 'folder') {
          secondSet.content = getSettingsByPriority(secondSet.content as Setting[]);
          const firstChild = (secondSet.content as Setting[])[0];
          secondPriority = ((firstChild.content as SettingContent).priority || 0);
        } else {
    ...
    if (Array.isArray(settings) && Array.isArray(ids)) {
      for (const setting of settings) {
        if (setting.type === 'folder') {
          const settingContent = filterSettingByIds(setting.content as Setting[], ids);
  ...
  const getDefaultSettings = (settings: Setting[]): Setting[] => {
    ...
    if (Array.isArray(settings)) {
      settings.forEach(setting => {
        if (setting.type === 'folder') {
          setting.content = getDefaultSettings(setting.content as Setting[]);
   ...
   */
  const getAvailableSettings = (settings: Setting[], ids: string[] = []): Setting[] => {
   ...
   * Get a setting by ID.
   */
  const getSettingById = (settings: Setting[], settingId: string): SettingContent | {} => {
    if (Array.isArray(settings)) {
      for (const setting of settings) {
        if (setting.type === 'folder') {
          const found = getSettingById(setting.content as Setting[], settingId);
